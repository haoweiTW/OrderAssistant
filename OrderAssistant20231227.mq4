...
//+------------------------------------------------------------------+
#property copyright "Copyright 2023 HaoWei"
...
#include <stdlib.mqh>
//MT5#include <MT4Orders.mqh>
//MT5#include <StdLibErr.mqh>
...
void DrawButton(string myName,string myText,string myFont,int myFontSize,color myColor,color myBackColor,int myX,int myY,int myWidth,int myHeight){
	...
	ObjectSetInteger(ChartIDLong,myName,OBJPROP_XSIZE,myWidth);
	ObjectSetInteger(ChartIDLong,myName,OBJPROP_YSIZE,myHeight);
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam){
	...
	if(id==CHARTEVENT_OBJECT_CLICK){
		...
		if(sparam=="OpenSell"){
			...
			BtnOpen(OP_SELL,LotsInit,SLInit,TPInit,Comments,"BtnSell Open");
		}
		if(sparam=="OpenBuy"){
			PlaySound("tick.wav");
			ObjectSetInteger(ChartIDLong,"OpenBuy",OBJPROP_STATE,false);
			BtnOpen(OP_BUY,LotsInit,SLInit,TPInit,Comments,"BtnBuy Open");
		}
		if(sparam=="CloseSell"){
			PlaySound("tick.wav");
			ObjectSetInteger(ChartIDLong,"CloseSell",OBJPROP_STATE,false);
			BtnClose(OP_SELL,"BtnSell Close");
		}
		if(sparam=="CloseBuy"){
			PlaySound("tick.wav");
			ObjectSetInteger(ChartIDLong,"CloseBuy",OBJPROP_STATE,false);
			BtnClose(OP_BUY,"BtnBuy Close");
		}
	}
}
...
void MarketHistory(){
	...
	for(HistoryCount=(OrdersHistoryTotal()-1);HistoryCount>=0;HistoryCount--){
		if(OrderSelect(HistoryCount,SELECT_BY_POS,MODE_HISTORY)==true && OrderMagicNumber()==ID){
...
void MarketCurrent(){
	...
	for(CurrentCount=(OrdersTotal()-1);CurrentCount>=0;CurrentCount--){
		if(OrderSelect(CurrentCount,SELECT_BY_POS,MODE_TRADES)==true && OrderMagicNumber()==ID){
			if(StringCompare(OrderSymbol(),Symbol())==0){
				if(OrderType()==OP_BUY){
					LotsBuy+=OrderLots();
...
double StopLoss;
double TakeProfit;
void BtnOpen(int myOP,double myLots,double mySL,double myTP,string myComment,string myMsg){
...
int CloseCount;
void BtnClose(int myOP,string myMsg){
	
	for(CloseCount=(OrdersTotal()-1);CloseCount>=0;CloseCount--){
		if(OrderSelect(CloseCount,SELECT_BY_POS,MODE_TRADES)==true && OrderMagicNumber()==ID){
			if(StringCompare(OrderSymbol(),Symbol())==0){
				if(OrderType()==myOP && myOP==OP_BUY){
				...
				if(OrderType()==myOP && myOP==OP_SELL){
					CloseOrderTicket=OrderTicket();
...
void MarketUI(){
	...
	if(MathAbs(LotsBuy-LotsSell)==0){
		...
		PriceBreakEven=NormalizeDouble((SymbolInfoDouble(Symbol(),SYMBOL_BID)-SymbolInfoDouble(Symbol(),SYMBOL_POINT)*(ProfitBuy+ProfitSell)/((LotsBuy-LotsSell)*SymbolInfoDouble(Symbol(),SYMBOL_TRADE_TICK_VALUE))),(int)SymbolInfoInteger(Symbol(),SYMBOL_DIGITS));
		
		if(ObjectFind(ChartIDLong,"BreakEven")>-1){
			ObjectMove(ChartIDLong,"BreakEven",0,TimeCurrent(),PriceBreakEven);
		}else{
			DrawHorizontalLine("BreakEven","",PriceBreakEven,clrAquamarine,STYLE_DASH,1);
		}
	}
	
	DrawButton("OpenSell","SELL","Times New Roman",13,clrWhite,clrRed,10,80*Scaling,70*Scaling,35);
	DrawButton("OpenBuy","BUY","Times New Roman",13,clrWhite,clrBlue,110*Scaling,80*Scaling,70*Scaling,35);
	
	DrawButton("CloseSell","Close","Times New Roman",14,clrWhite,clrCrimson,10,130*Scaling,70*Scaling,35);
	DrawButton("CloseBuy","Close","Times New Roman",14,clrWhite,clrMediumBlue,110*Scaling,130*Scaling,70*Scaling,35);
	
	DrawLabel("DailyReturn","DailyReturn  "+DoubleToString(ReturnSum,2),"Times New Roman",14,clrGold,20,200*Scaling);
	
	DrawLabel("LotsSell","v "+DoubleToString(LotsSell,2),"Times New Roman",14,clrDeepPink,15,230*Scaling);
	DrawLabel("ProfitSell","$ "+DoubleToString(ProfitSell,2),"Times New Roman",14,clrDeepPink,15,250*Scaling);
	
	DrawLabel("LotsBuy","v "+DoubleToString(LotsBuy,2),"Times New Roman",14,clrDeepSkyBlue,115,230*Scaling);
	DrawLabel("ProfitBuy","$ "+DoubleToString(ProfitBuy,2),"Times New Roman",14,clrDeepSkyBlue,115,250*Scaling);
}
